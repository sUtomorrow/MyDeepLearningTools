...
def data_annotations2input_outputs(max_gts=100, convert_ndarray=False):
    def _data_annotations2input_outputs(data, annotations):
        ...
        for bbox, class_idx in zip(annotations['bboxes'], annotations['class_idxes']):
            ...
            gt_class_idxes.append(class_idx)

        gt_boxes = np.array(gt_boxes, dtype=np.float32)
        gt_class_idxes = np.array(gt_class_idxes, dtype=np.int32)

        if len(gt_boxes) < max_gts:
            gt_boxes = np.pad(gt_boxes, ((0, max_gts - len(gt_boxes)), (0, 0)), mode='constant', constant_values=0)
            gt_class_idxes = np.pad(gt_class_idxes, ((0, max_gts - len(gt_class_idxes))), mode='constant', constant_values=-1)
        elif len(gt_boxes) > max_gts:
            indeices = [True for _ in range(max_gts)] + [False for _ in range(len(gt_boxes) - max_gts)]
            gt_boxes = gt_boxes[indeices, :]
            gt_class_idxes = gt_class_idxes[indeices]

        return image_inputs, gt_boxes, gt_class_idxes
    return _data_annotations2input_outputs
